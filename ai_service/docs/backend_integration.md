...
## 7. 콜백(웹훅) — 요청 주신 사항, AI 쪽 반영 필요

지금까지는 백엔드가 `/status`를 폴링하는 구조였는데, 완료 시점에 세션 참여자에게 웹소켓으로
바로 알려주기 위해 **콜백을 추가로 요청**드립니다. 폴링(§2)은 콜백이 유실됐을 때의 안전망으로
백엔드 쪽에 그대로 남겨뒀으니, 콜백이 실패하거나 늦어도 최종적으로는 폴링이 따라잡습니다 —
그래서 콜백 전송에 실패해도 재시도까지는 필요 없고, 한 번 시도하고 실패하면 넘어가도 됩니다.
...
```json
{
  "mode": "ASSESSMENT",
  "...": "...",
  "callback_url": "http://<backend-host>/api/quiz/42/callback"
}
```

| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `callback_url` | string\|null | 아니오 | 있으면 파이프라인 종료(READY/FAILED) 시 이 주소로 결과를 POST. 없으면 기존처럼 폴링만으로 동작 |

### 7.2 콜백 요청 — `/status` 응답과 완전히 같은 모양

파이프라인이 끝나면(READY 또는 FAILED) `callback_url`로 **`GET /api/quiz/{id}/status`가 돌려주는 것과
동일한 JSON**을 그대로 POST 해주시면 됩니다. 별도 스키마를 새로 만들 필요 없이 이미 있는 응답
직렬화를 재사용하면 됩니다. PENDING/GENERATING 상태는 보낼 필요 없습니다(폴링이 이미 그 상태를 전제로 동작).

```
POST {callback_url}
Content-Type: application/json

{ "project": "201", "quiz_set_id": 42, "status": "READY", "quizzes": [...], "error_message": null }
```

백엔드는 `204`로 응답하고 본문은 보지 않습니다. 콜백 요청 자체가 실패(연결 거부, 타임아웃 등)해도
예외를 삼키고 넘어가면 됩니다 — 어차피 백엔드가 폴링으로 뒤늦게 따라잡습니다.

### 7.3 인증 헤더 (선택)

배포 값이 설정되면 백엔드가 아래 헤더를 검사합니다. 로컬 개발 중에는 백엔드 쪽 값이 비어있어
검사를 건너뛰므로 당장은 안 보내도 됩니다. 배포 전에 공유 비밀 값을 맞춰야 합니다.

```
X-Ai-Callback-Secret: <공유 비밀>
```
