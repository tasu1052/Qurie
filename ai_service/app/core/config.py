...
# --- 데모 모드 ---
# 발표 시연용. 켜면 교차검증(SOLVE·JUDGE)을 건너뛰고 생성분을 그대로 내보낸다.
# 요청 개수를 채우는 것이 목적이라 품질 보증이 없다. 다른 모델이 못 푸는 문항도,
# 정답이 틀린 문항도 그대로 나간다. **운영 기본값으로 두지 말 것.**
#
#   켜기: AI_DEMO_MODE=1 로 서버 기동
#   끄기: 환경변수 제거 후 재기동 (코드는 되돌릴 것이 없다)
#
# 구간 중복 판정도 함께 느슨해진다. 같은 줄을 다른 각도로 묻는 문항이 통째로
# 폐기되면서 재생성 라운드가 전멸하는 것이 개수 미달의 주원인이기 때문이다.
# 문장 유사도 판정은 그대로 살아 있어 눈에 보이는 중복 문항은 여전히 걸러진다.
...
# 요청 개수보다 이만큼 더 생성한다.
# GENERATE 입력은 코드 전문이라 라운드마다 똑같이 나가고 문항 수는 출력에만 영향을
# 준다 — 라운드를 한 번 더 도는 것보다 한 라운드에서 몇 개 더 뽑는 쪽이 싸다.
# 넘치면 judge_score 높은 순으로 잘라 요청 개수만 내보낸다.
QUIZ_OVERSHOOT = 3
